  ...
  <div class="container my-5">
    ...
    <form id="plantillaForm" action="{{ route('empresa.plantilla.submit') }}" method="POST" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        ...
        <div class="d-flex gap-3">
          <div class="w-100">
            ...
            <input type="color" name="color_primario" id="color_primario" class="form-control form-control-color" value="{{ old('color_primario','#ff0000') }}">
          </div>
          <div class="w-100">
            <label for="color_secundario" class="form-label small">Secundario</label>
            <input type="color" name="color_secundario" id="color_secundario" class="form-control form-control-color" value="{{ old('color_secundario','#00ff00') }}">
          </div>
          <div class="w-100">
            <label for="color_terciario" class="form-label small">Terciario</label>
            <input type="color" name="color_terciario" id="color_terciario" class="form-control form-control-color" value="{{ old('color_terciario','#0000ff') }}">
          </div>
        </div>
      </div>
      <!-- Botones -->
      <div class="d-flex gap-2 mt-4">
        ...
        <button type="submit" id="btn-pagar" class="btn btn-success" {{ empty($empresa->datos_fiscales_id) ? 'disabled' : '' }}>
