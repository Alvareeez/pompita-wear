...
<div class="container-fluid mt-4">
  <div class="row g-4">
    ...
    <div class="col-md-6">
      <form action="{{ route('perfil.update') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
          ...
          <input type="email" disabled name="email" id="email" class="form-control" value="{{ $user->email }}">
        </div>
        <div class="row">
          ...
          <div class="col-md-6 mb-3">
            ...
            <input type="password" name="password_confirmation" id="password_confirmation" class="form-control" placeholder="Repetir contraseña">
          </div>
        </div>
        <div class="mb-3">
      ...
      @if(Auth::check() && Auth::id() !== $user->id_usuario)
        @php
          ...
          $hasPending  = Auth::user()->solicitudesEnviadas()->where('id_receptor', $user->id_usuario)->where('status','pendiente')->exists();
        @endphp
        <form action="{{ route('solicitudes.store') }}" method="POST" class="text-center my-3">
          ...
          <button type="submit" class="btn btn-primary w-75" {{ $isFollowing || $hasPending ? 'disabled' : '' }}>
    ...
    {{-- Columna derecha: solicitudes + seguidores/seguidos --}}
    <div class="col-md-6">
      @if($user->is_private)
        <div class="card mb-4">
            ...
            @else
              @foreach($pendientes as $sol)
                <div class="d-flex align-items-center justify-content-between mb-3">
                  <a href="{{ route('perfil.publico', $sol->emisor->id_usuario) }}" class="d-flex align-items-center text-decoration-none">
                  ...
                  <div>
                    <form action="{{ route('solicitudes.aceptar', $sol->id) }}" method="POST" class="d-inline">@csrf
                      <button class="btn btn-sm btn-success"><i class="fas fa-check"></i></button>
                    </form>
                    <form action="{{ route('solicitudes.rechazar', $sol->id) }}" method="POST" class="d-inline ms-2">@csrf
              ...
              @forelse($user->seguidores as $f)
                <div class="d-flex align-items-center justify-content-between mb-2">
                  <div class="d-flex align-items-center">
                    ...
                    <a href="{{ route('perfil.publico', $f->id_usuario) }}">{{ $f->nombre }}</a>
                  </div>
                ...
                <div class="d-flex align-items-center justify-content-between mb-2">
                  <div class="d-flex align-items-center">
                    <img src="{{ $s->foto_perfil ? asset($s->foto_perfil) : asset('img/default-profile.png') }}"
                         width="40" height="40" class="rounded-circle me-2">
                    <a href="{{ route('perfil.publico', $s->id_usuario) }}">{{ $s->nombre }}</a>
                  </div>
  ...
  {{-- Sección inferior: outfits y favoritos --}}
  <div class="row">
    <div class="col-12">
      <h2 class="mb-4">Mis Outfits</h2>
